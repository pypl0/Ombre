...
class ComputeAgent:
    ...
    def process(self, ctx: Any) -> Any:
        ...
        ctx.estimated_cost_without_ombre = self._estimate_raw_cost(estimated_input_tokens, model)
        ...
        for provider in providers_to_try:
            try:
                ...
                if provider != ctx.selected_provider:
                    logger.info(f"Fallback succeeded | request={ctx.request_id} | provider={provider}")
                    ctx.selected_provider = provider
                ...
                ctx.cost_saved += max(0, ctx.estimated_cost_without_ombre - actual_cost)
    ...
    def _select_model(self, preferred_model: str, task_type: str) -> tuple:
        if preferred_model != "auto":
            ...
            logger.warning(f"Preferred model {preferred_model} provider not available, falling back to auto-selection")

        preferred_models = self.config.get_preferred_models(task_type)
        for model in preferred_models:
            provider = self.config._get_model_provider(model)
            if provider in self.config.available_providers:
                if self._provider_failures.get(provider, 0) < 3:
                    return model, provider

        if self.config.available_providers:
            provider = self.config.available_providers[0]
            fallback_models = {
                "openai": "gpt-4o-mini",
            ...
            model = fallback_models.get(provider, "gpt-4o-mini")
            return model, provider
    ...
    def _call_provider(self, provider, model, messages, system, temperature, max_tokens):
        if provider == "openai":
    ...
    def _call_openai(self, model, messages, system, temperature, max_tokens):
        try:
    ...
    def _call_anthropic(self, model, messages, system, temperature, max_tokens):
        try:
            import anthropic
        except ImportError:
    ...
    def _call_groq(self, model, messages, system, temperature, max_tokens):
        try:
    ...
    def _call_mistral(self, model, messages, system, temperature, max_tokens):
        try:
    ...
    def _embed_cohere(self, text: str) -> List[float]:
        ...
        response = client.embed(texts=[text], model="embed-english-v3.0", input_type="search_query")
